...
if [ "$CI" = true ]; then
    ...
    # NOTE: cloning feathers-import-export could be avoided if we could parse app_version from tag/branch name instead
    # In this case, the kli would clone feathers-import-export
    ...
    git clone --depth 1 $GIT_OPS "$GITHUB_URL/kalisio/feathers-import-export.git" "$WORKSPACE_DIR/feathers-import-export"
