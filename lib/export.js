...
export async function _export (options) {
  ...
  const transformation = options.transform
  // define a unique output file
  const timestamp = Date.now().toString()
  let tmpFile = path.join(options.workingDir, `${timestamp}-${options.filename}`)
  ...
  while (info.currentChunk < info.totalChunks) {
    ...
    if (chunk) {
      ...
      if (transformation) chunk = transform(chunk, transformation)
      debug(`Writing ${_.size(chunk)} objects`)
      await write(outputSteam, exporter.process(info, chunk), options.gzip)
    }
    info.currentChunk++
  }
  // end the process
  ...
  Object.assign(response, { chunks: info.totalChunks, objects: info.objectCount })
