...
export async function _export (options) {
  ...
  // create a write stream using uuid
  let outputSteam = fs.createWriteStream(options.uuid)
  // export the data
  const writeResult = await _writeData(outputSteam, options)
  // wait for a lapse of time
  debug(`Waiting for a lapse of time before uploading exported file`)
  await new Promise(resolve => setTimeout(resolve, 1000))
  // upload the generated file
  debug(`Uploading texport exported file ${options.uuid}`)
  let response = await options.s3Service.uploadFile({ 
    filePath: options.uuid, 
