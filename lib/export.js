...
export async function _export (options) {
  ...
  while (info.currentChunk < info.totalChunks) {
    ...
    debug(`Querying service from ${offset} with limit ${options.chunkSize}`)
    response = await options.service.find({ query: Object.assign(options.query, { $limit: options.chunkSize, $skip: offset }) })
