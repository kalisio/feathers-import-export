...
export class Service {
  ...
  async create (data, params) {
    if (!data.method) throw new BadRequest('create: missing \'data.action\' parameter')
    switch (data.method) {
      case 'import': 
  ...
  async import (data, params) {
    ...
    // retrieve the object detail
    const response = await this.s3Service.getObjectCommand({  id })
  ...
  async export (data, params) {
    ...
    let tmpFile = path.join(this.workingDir, `${serviceName}.${format}`)
    if (zip) tmpFile += '.gz'
    ...
    response = await this.s3Service.uploadFile({ filePath: tmpFile, mimeType: zip ? 'application/gzip' : exporter.type()})
    debug(`Uploaded temporary file ${tmpFile} with key ${response.id}`)
    // removed output file
    //fs.unlinkSync(tmpFile)
    debug(`Deleted temporary file ${tmpFile}`)
